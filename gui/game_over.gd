...
var final_score := 0
## Regex strips non-printable ASCII — same rule used in database.gd.
var _name_regex := RegEx.new()


func _ready() -> void:
	_name_regex.compile("[^ -~]")  # strip anything outside printable ASCII
	name_line_edit.max_length = 20   # enforce at the UI level too
	...
	final_score = Globals.score_distance
	final_score += Globals.score_coins
	final_score += Globals.score_jump
	final_score += Globals.score_backflip
	final_score += Globals.score_medals * Globals.MEDAL_VALUE
	your_score_label.text = str(final_score)
	name_line_edit.grab_focus()


func save_score_to_db() -> void:
	var raw_name := name_line_edit.text
	# Strip non-printable ASCII, trim whitespace, fall back to "Player".
	var player_name := _name_regex.sub(raw_name, "", true).strip_edges()
	if player_name.is_empty():
		player_name = "Player"
	Db.insert_score(player_name, final_score)
	# Persistent coin aggregation
	Globals.coins += Globals.score_coins
	Globals.save_config()
