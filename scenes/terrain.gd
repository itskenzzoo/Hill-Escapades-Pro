...
const FUEL := preload("res://items/fuel.tscn")
...
func _create_terrain_section(sec : int) -> void:
	...
	var angles_arr  : Array[float] = []  # slope angle per point, used for width curve
	...
	line.begin_cap_mode = Line2D.LINE_CAP_ROUND  # round both ends for clean section joins
	line.end_cap_mode = Line2D.LINE_CAP_ROUND
	line.texture_filter = CanvasItem.TEXTURE_FILTER_NEAREST
	line.antialiased = true  # removes staircase artefacts on diagonal slopes
	line.width = 96  # slightly reduced base; curve multiplier handles the rest
	line.points = points_line
	line.z_index = 2 # La línea siempre debe dibujarse arriba del polígono

	# Build a width curve: thickest on flat ground, thinner on steep slopes.
	# abs(sin(angle)) == 0 on flat, 1 on vertical; we map that to width [0.70 .. 1.0].
	var width_curve := Curve.new()
	var n_pts := angles_arr.size()
	for i in n_pts:
		var t          := float(i) / float(max(n_pts - 1, 1))
		var steepness  := absf(sin(angles_arr[i]))
		var w_factor   := lerpf(1.0, 0.70, steepness)
		width_curve.add_point(Vector2(t, w_factor))
	...
	# Puntos de fuel
	if generate_items and sec % 2 == 0 and sec > 0:
		var fuel = FUEL.instantiate()
		fuel.position.x = 4890
		fuel.position.y = get_position_y(4890 + sec * SECTION_POINTS * POINT_SIZE) - 900
